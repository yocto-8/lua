...
const vm_handler vm_handler_table[41] = {
    vmOP_MOVE,
    vmOP_LOADK,
    vmOP_LOADKX,
    vmOP_LOADBOOL,
    ...
    vmOP_ADD,/*	A B C	R(A) := RK(B) + RK(C)				*/
    vmOP_SUB,/*	A B C	R(A) := RK(B) - RK(C)				*/
    vmOP_MUL,/*	A B C	R(A) := RK(B) * RK(C)				*/
    vmOP_DIV,/*	A B C	R(A) := RK(B) / RK(C)				*/
    vmOP_IDIV,/*	A B C	R(A) := RK(B) \ RK(C)				*/
    vmOP_MOD,/*	A B C	R(A) := RK(B) % RK(C)				*/
    vmOP_POW,/*	A B C	R(A) := RK(B) ^ RK(C)				*/
    vmOP_UNM,/*	A B	R(A) := -R(B)					*/
    vmOP_NOT,/*	A B	R(A) := not R(B)				*/
    vmOP_LEN,/*	A B	R(A) := length of R(B)				*/

    vmOP_CONCAT,/*	A B C	R(A) := R(B).. ... ..R(C)			*/

    vmOP_JMP,/*	A sBx	pc+=sBx; if (A) close all upvalues >= R(A - 1)	*/
    vmOP_EQ,/*	A B C	if ((RK(B) == RK(C)) ~= A) then pc++		*/
    vmOP_LT,/*	A B C	if ((RK(B) <  RK(C)) ~= A) then pc++		*/
    vmOP_LE,/*	A B C	if ((RK(B) <= RK(C)) ~= A) then pc++		*/

    vmOP_TEST,/*	A C	if not (R(A) <=> C) then pc++			*/
    vmOP_TESTSET,/*	A B C	if (R(B) <=> C) then R(A) := R(B) else pc++	*/

    vmOP_CALL,/*	A B C	R(A), ... ,R(A+C-2) := R(A)(R(A+1), ... ,R(A+B-1)) */
    vmOP_TAILCALL,/*	A B C	return R(A)(R(A+1), ... ,R(A+B-1))		*/
    vmOP_RETURN,/*	A B	return R(A), ... ,R(A+B-2)	(see note)	*/

    vmOP_FORLOOP,/*	A sBx	R(A)+=R(A+2);
          if R(A) <?= R(A+1) then { pc+=sBx; R(A+3)=R(A) }*/
    vmOP_FORPREP,/*	A sBx	R(A)-=R(A+2); pc+=sBx				*/

    vmOP_TFORCALL,/*	A C	R(A+3), ... ,R(A+2+C) := R(A)(R(A+1), R(A+2));	*/
    vmOP_TFORLOOP,/*	A sBx	if R(A+1) ~= nil then { R(A)=R(A+1); pc += sBx }*/
